...
import { X, Send, CreditCard, Heart, Phone, UserPlus, Trash2, PlusCircle } from 'lucide-react';
...
function SubmitButton({ shareCount }: { shareCount: number }) {
  ...
  return (
    ...
    >
      {pending ? (
        ...
        <>
          <Send size={18} />
          Book {shareCount} {shareCount === 1 ? 'Share' : 'Shares'}
        </>
      )}
    </button>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      ...
      >
        ...
        <div className="p-8 pb-4 shrink-0">
          ...
          <p className="text-foreground/60 text-sm">One person can book multiple shares for their family.</p>
        </div>
          ...
          <form action={handleSubmit} ref={formRef} className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              <section>
                ...
                <select
                  name="distribution"
                  className="w-full bg-foreground/5 border-2 border-transparent focus:border-secondary/30 focus:bg-white p-4 rounded-2xl outline-none appearance-none transition-all duration-300 cursor-pointer"
                >
            ...
            <div className="p-5 bg-secondary/5 rounded-[2.5rem] border border-secondary/10">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-sm font-bold text-primary mb-1 tracking-tight">Payment: Br. Mustafizur Rahman</h4>
                  <p className="text-[11px] text-foreground/50 leading-relaxed font-medium">
                    Please send total for **{beneficiaries.length} shares** to:<br/>
            ...
            <SubmitButton shareCount={beneficiaries.length} />
          </form>
        </div>
      </motion.div>
    </div>
